...
const OLLAMA_ANSWERS_PATH = path.join(ROOT, "scripts", "answer-bank", "actual-interview-ollama-updated-answers.json");
...
function buildDefaultAnswer(entry) {
  ...
  return [
    ...
    `Use a concrete example from GCP, Kubernetes, Terraform, CI/CD, SRE, security, networking, or platform engineering depending on the question: ${entry.question}`
  ].join(" ");
}

function addUnique(entries, sourceEntries, answerByQuestion) {
  const seen = new Set(entries.map((entry) => normalize(entry.question)));
  for (const entry of sourceEntries) {
    const key = normalize(entry.question);
    if (!key || seen.has(key)) continue;
    entries.push({
      ...entry,
      answer: entry.answer || answerByQuestion.get(key) || buildDefaultAnswer(entry)
    });
    seen.add(key);
  }
}

const ollamaAnswers = fs.existsSync(OLLAMA_ANSWERS_PATH)
  ? JSON.parse(fs.readFileSync(OLLAMA_ANSWERS_PATH, "utf8"))
  : {};
...
for (const [key, answer] of Object.entries(ollamaAnswers)) {
  if (key && answer) {
...
console.log(`Exported ${allEntries.length} questions.`);
console.log(`Questions file: ${QUESTIONS_OUT}`);
console.log(`QA file: ${QA_OUT}`);
